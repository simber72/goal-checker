...
public final class SSO_CommandLine_Parser extends AnBx_CommandLine_Parser<SSO_Roles,SSO_Channels> {

	public SSO_CommandLine_Parser(String[] args, String prot) {
		super(args, prot, prot, SSO_Roles.class, true);
	...
	protected void initProtocol() {
		...
		if (myAlias != null && keypath != null && sharepath != null) {
			switch (role) {
			case ROLE_SP:
				SSO_pr = new SSO_Principal(myAlias, keypath, cs, aliases, true, crypto_config);
                SSO_pr.run(new SSO_ROLE_SP(role, protname, sharepath), sessions);
				break;
			case ROLE_idp:
				SSO_pr = new SSO_Principal(myAlias, keypath, cs, aliases, true, crypto_config);
                SSO_pr.run(new SSO_ROLE_idp(role, protname, sharepath), sessions);
				break;
			case ROLE_C:
				SSO_pr = new SSO_Principal(myAlias, keypath, cs, aliases, true, crypto_config);
                SSO_pr.run(new SSO_ROLE_C(role, protname, sharepath), sessions);
				break;
